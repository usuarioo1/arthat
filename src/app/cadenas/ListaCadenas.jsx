...
import { apiUrlCadenas } from "@/utils/api";

const getData = async () => {
    const res = await fetch(apiUrlCadenas);
...
const ListaCadenas = () => {
    const [cadenasFetch, setCadenasFetch] = useState([]);

    useEffect(() => {
        const fetchCadenas = async () => {
            try {
                const data = await getData();
                setCadenasFetch(data.info);
            } catch (error) {
    ...
    return (
        <div className="flex flex-wrap justify-center mt-20">
            {cadenasFetch.map((producto, index) => (
